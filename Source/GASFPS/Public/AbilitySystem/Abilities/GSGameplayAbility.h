...
/**
 * EGSAbilityActivationGroup
 *
 *	Defines how an ability activates in relation to other abilities.
 */
UENUM(BlueprintType)
enum class EGSAbilityActivationGroup : uint8
{
...
struct FGSAbilityMontageFailureMessage
{
	GENERATED_BODY()
...
public:
	...
	UGSAbilitySystemComponent* GetGSAbilitySystemComponentFromActorInfo() const;
	...
	UFUNCTION(BlueprintCallable, Category = "GS|Ability")
	UGSHeroComponent* GetHeroComponentFromActorInfo() const;

	EGSAbilityActivationPolicy GetActivationPolicy() const { return ActivationPolicy; }
	EGSAbilityActivationGroup GetActivationGroup() const { return ActivationGroup; }

	void TryActivateAbilityOnSpawn(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec) const;

	// Returns true if the requested activation group is a valid transition.
	UFUNCTION(BlueprintCallable, BlueprintPure = false, Category = "GS|Ability", Meta = (ExpandBoolAsExecs = "ReturnValue"))
	bool CanChangeActivationGroup(EGSAbilityActivationGroup NewGroup) const;

	// Tries to change the activation group.  Returns true if it successfully changed.
	UFUNCTION(BlueprintCallable, BlueprintPure = false, Category = "GS|Ability", Meta = (ExpandBoolAsExecs = "ReturnValue"))
	bool ChangeActivationGroup(EGSAbilityActivationGroup NewGroup);
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "GS|Ability Activation")
	EGSAbilityActivationGroup ActivationGroup;
