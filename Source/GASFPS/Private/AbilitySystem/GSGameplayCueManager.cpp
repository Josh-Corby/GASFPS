...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GSGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EGSEditorLoadMode
{
...
namespace GSGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("GS.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via GSGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UGSGameplayCueManager::DumpGameplayCues));

	static EGSEditorLoadMode LoadMode = EGSEditorLoadMode::LoadUpfront;
}
...
UGSGameplayCueManager::UGSGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGSGameplayCueManager* UGSGameplayCueManager::Get()
{
	return Cast<UGSGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UGSGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UGSGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UGSGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UGSGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (GSGameplayCueManagerCvars::LoadMode)
	{
	case EGSEditorLoadMode::LoadUpfront:
		return true;
	case EGSEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EGSEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UGSGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UGSGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UGSGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UGSGameplayCueManager* GCM = Cast<UGSGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogTemp, Error, TEXT("DumpGameplayCues failed. No UGSGameplayCueManager found."));
		return;
	}
...
void UGSGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UGSGameplayCueManager* StrongThis = Get())
					{
...
void UGSGameplayCueManager::HandlePostGarbageCollect()
{
...
void UGSGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UGSGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UGSGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (GSGameplayCueManagerCvars::LoadMode)
	{
	case EGSEditorLoadMode::LoadUpfront:
...
void UGSGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UGSGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UGSGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UGSGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UGSGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UGSGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
